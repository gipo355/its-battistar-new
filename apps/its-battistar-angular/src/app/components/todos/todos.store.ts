        ...
        filteredTodos: computed(() => {
            ...
            if (filter.currentSortBy() === 'DueDate') {
                filteredTodos = filteredTodos.sort((todoPrev, todoNext) => {
                    ...
                    return (
                        todoPrev.dueDate.getTime() - todoNext.dueDate.getTime()
                    );
                });
            }
            ...
            if (filter.currentSortBy() === 'Newest') {
                filteredTodos = filteredTodos.sort(
                    (todoPrev, todoNext) =>
                        todoNext.createdAt.getTime() -
                        todoPrev.createdAt.getTime()
                );
            }
    ...
    withMethods((store, todoService = inject(TodosService)) => ({
        ...
        async loadTodos(): Promise<void> {
            try {
                ...
                if (!request.ok) {
                    ...
                    throw new Error(
                        `Error loading todos: ${request.message ?? ''}`
                    );
                }
        ...
        async createTodo(): Promise<void> {
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `Error creating todo: ${response.message ?? ''}`
                    );
                }
        ...
        async deleteTodo(): Promise<void> {
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `Error deleting todo: ${response.message ?? ''}`
                    );
                }
        ...
        async updateTodo(): Promise<void> {
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `Error updating todo: ${response.message ?? ''}`
                    );
                }
